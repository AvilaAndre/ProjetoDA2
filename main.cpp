...
void Situation11(Helper helper) {
    ...
    graph.dijkstraHighestCapacityPath(s);
    std::vector<int> roadPath = graph.getPath(s, t);
    std::cout << "Path: ";
    ...
    std::cout << "Maximum capacity: " << graph.getVertex(graph.getNumVertex()).getDist() << std::endl;
}
...
void Situation12(Helper helper) {
    ...
    graph.dijkstraHighestCapacityPath(s);
    std::vector<int> roadPathA = graph.getPath(s, t);
    ...
    graph.BFS(s);
    std::vector<int> roadPathB = graph.getPath(s, t);
...
void Situation24(Helper helper) {
    ...
    std::cout << "Starting point: 1." << std::endl;
    ...
    std::cout << "The group would meet again at the destination after " << ans1.second[t] << " hours, minimum." << std::endl;
}
...
void Situation25(Helper helper) {
    ...
    std::cout << "Finishing point: " << graph.getNumVertex() << "." << std::endl;
